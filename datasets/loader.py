...
# 		source_img = read_img(os.path.join(self.root_dir, 'hazy', img_name).replace('\\', '/')) * 2 - 1
# 		target_img = read_img(os.path.join(self.root_dir, 'GT', img_name).replace('\\', '/')) * 2 - 1
...
#         self.img_num = len(self.img_names)
#
#         print("==> Preloading images into memory (this may take a minute)...")
#         self.source_imgs = []
#         self.target_imgs = []
#         for name in self.img_names:
#             hazy = read_img(os.path.join(self.root_dir, 'hazy', name)) * 2 - 1
#             gt = read_img(os.path.join(self.root_dir, 'GT', name)) * 2 - 1
#             self.source_imgs.append(hazy)
#             self.target_imgs.append(gt)
...
#
#     def __getitem__(self, idx):
#         source_img = self.source_imgs[idx].copy()
#         target_img = self.target_imgs[idx].copy()
#
#         if self.mode == 'train':
#             [source_img, target_img] = augment([source_img, target_img], self.size, self.edge_decay, self.only_h_flip)
#
...
#             [source_img, target_img] = align([source_img, target_img], self.size)
#
#         return {'source': hwc_to_chw(source_img), 'target': hwc_to_chw(target_img), 'filename': self.img_names[idx]}
class PairLoader(Dataset):
    def __init__(self, data_dir, sub_dir, mode, size=256, edge_decay=0, only_h_flip=False):
        assert mode in ['train', 'valid', 'test']
...
class SingleLoader(Dataset):
	...
	def __getitem__(self, idx):
		...
		# read image, and scale [0, 1] to [-1, 1]
		img_name = self.img_names[idx]
		img = read_img(os.path.join(self.root_dir, img_name).replace('\\', '/')) * 2 - 1
